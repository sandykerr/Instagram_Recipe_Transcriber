...
class RuleBasedRecipeExtractor:
    ...
    version = "rule-based-recipe-extractor-v3"
    ...
    def extract(
        ...
        conflicts = self._find_quantity_conflicts(ingredients)
    ...
    @staticmethod
    def _find_quantity_conflicts(ingredients: tuple[Ingredient, ...]) -> tuple[str, ...]:
        values_by_name: dict[str, set[tuple[str | None, str | None]]] = defaultdict(set)
        for ingredient in ingredients:
            values_by_name[_normalized(ingredient.name)].add(
                (ingredient.quantity_original, ingredient.unit_original)
            )
        return tuple(
            f"Conflicting explicit quantities for ingredient: {name}"
...
class RecipeValidator:
    ...
    def version(self) -> str:
        return f"recipe-validator-v2:minimum-instructions={self._minimum_instruction_count}"
    ...
    def validate(self, recipe: RecipeCandidate) -> ValidationArtifact:
        ...
        for ingredient in recipe.ingredients:
            ...
            if ingredient.quantity_original is None:
                findings.append(
                    _finding(
                        "missing_quantity",
                        f"Ingredient has no explicit quantity: {ingredient.name}",
                    )
                )
            if re.match(r"^(?:\d|[¼½¾⅓⅔])", ingredient.name):
                findings.append(
                    _finding(
